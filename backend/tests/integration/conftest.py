...
def run_migrations(engine):
    ...
    # Dynamically resolve the path to backend/alembic based on this file's location
...
def clean_and_seed_db(engine):
    ...
    # 2. Re-seed the manager for this specific test
...
def seed_waiter(client, seed_manager) -> dict:
    resp = client.post(
        "/staff",
        json={"name": "Test Waiter", "pin": "5678", "role": "waiter"},
...
def seed_menu_item(client, seed_manager) -> dict:
    """Create a single menu item as manager."""
    resp = client.post(
        "/menu",
...
@pytest.fixture
def seed_table(client, seed_manager) -> dict:
    """Create a single table as manager."""
    resp = client.post(
        "/tables",
        json={"name": "Table 1", "section": "DINE-IN"},
        headers={"X-Staff-Id": seed_manager["id"]},
    )
    assert resp.status_code in (200, 201)
    return resp.json()
