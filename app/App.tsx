...
export default function App() {
  ...
  const { staff, setStaff, setDevice, deviceId } = useAuthStore();
  ...
  useEffect(() => {
    (async () => {
      ...
      // Restore device identity if this tablet has already been registered.
      ...
      // Per-device salt for the local PIN verifier. Generated once.
  ...
  if (!deviceId) return <><StatusBar style="dark" /><SetupScreen /></>;
  if (!staff) return <><StatusBar style="dark" /><PinLoginScreen /></>;
    ...
    return (
      <>
        ...
        <PaymentScreen
          ...
          onDone={() => { setPaymentOrderId(null); setTab("orders"); }}
        />
      </>
    );
  }
  ...
  if (showAdmin) {
    return (
      <>
        <StatusBar style="dark" />
        <MenuAdminScreen />
        <TouchableOpacity style={styles.back} onPress={() => setShowAdmin(false)}>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={styles.tabbar}>
        <TabBtn label="Menu" active={tab === "menu"} onPress={() => setTab("menu")} />
        <TabBtn label="Orders" active={tab === "orders"} onPress={() => setTab("orders")} />
        <TabBtn label="Settings" active={tab === "settings"} onPress={() => setTab("settings")} />
        {staff.role === "manager" && (
          <TabBtn label="Admin" active={false} onPress={() => setShowAdmin(true)} />
        )}
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  back: {
    ...
    backgroundColor: "#0f172a", paddingHorizontal: 24,
