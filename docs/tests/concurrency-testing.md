...
| Known risk | `create_cash_payment` and `void_order` both read the order with `db.get(...)` and do not take a row lock. A read-check-write race can produce `status = "void"` together with `payment_status = "paid"`. This test is expected to **fail** until the order row is locked (`with_for_update()`) or an equivalent guard is added. |
...
|---|---|---|
| `BASE_URL` | Backend origin under test | `http://localhost:8000` |
| `TEST_MANAGER_PIN` | PIN used for the void traffic slice | empty (void slice skipped) |
| `DEVICE_PREFIX` | Prefix for load-test devices | `LT` |
| `SUMMARY_PATH` | Markdown summary output path | `/tmp/MobileToServer-POS-load/load-summary.md` |

### Summary Shape

```markdown
## ✅ MobileToServer-POS Load Test Results
| Metric | Value | Threshold | Status |
|---|---:|---|---|
| p(95) Latency | 142.10 ms | < 300 ms | ✅ Pass |
| p(99) Latency | 388.44 ms | < 800 ms | ✅ Pass |
| Error Rate | 0.00% | < 0.5% | ✅ Pass |
| Duplicate order numbers | 0 | 0 | ✅ Pass |
| Paid + void orders | 0 | 0 | ✅ Pass |

*Total requests: 84213*
```
...
- Database connection-pool saturation and exhaustion
- Postgres lock-wait time and deadlock counts
- Container CPU / memory / disk saturation
- Growth of the `idempotency_keys` table under sustained write volume
- Per-endpoint latency breakdown
- Multi-venue (multi-tenant) isolation — MobileToServer-POS is single-venue
- Behaviour when the backend is unreachable mid-load (covered in [offline-testing.md](offline-testing.md))
